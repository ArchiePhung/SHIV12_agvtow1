...
class ros_control():
	...
	def synthetic_error(self):
		...
		# -- add 31/10/2024 - CPU > 90%
		if (self.data_NucInfo.cpu_usage >= 90):
	...
	def run(self):
		if self.process == -1: # khi moi khoi dong len
			...
			if self.app_button.bt_clearError == 1 or self.main_info.stsButton_reset == 1:
				...
				# -- add 23/7/2024
				if (self.find_element(241, self.listError) == 0 and self.find_element(242, self.listError) == 0):
					self.update_QT()
	...
	# -- RUN AUTO:
			...
			if self.flag_error == 1: 
				...
				elif self.find_element(293, self.listError) == 1:
				...
				elif (self.find_element(241, self.listError) == 1 or self.find_element(242, self.listError) == 1) and self.is_firsttime_odomNav == False:
