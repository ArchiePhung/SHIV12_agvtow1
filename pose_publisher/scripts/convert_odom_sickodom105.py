...
, self.callback_lls) # 
    ...
    ############################################ DEF FUNCTION ########################################################################3
    def pub_SickOdom(self, sick_odom , rate):

        if rospy.get_time() - self.time_tr > float(1/rate) : # < 20hz 
            self.time_tr = rospy.get_time()
            self.pub_sick_odom.publish(sick_odom)
        else :
            pass
        ...
        try:
            if self.is_exit == 0:
                while not rospy.is_shutdown():
                    if self.is_recv_odom == True and self.is_recv_lls == True:

                        self.sick_odom_data.header = self.odom_data.header
                        self.sick_odom_data.header.frame_id = "base_footprint"
                        ...
                        # self.pub_SickOdom(self.sick_odom_data, self.rate_sickOdom)
