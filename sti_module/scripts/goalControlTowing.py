...
# Hoang sua code cho AAGV keo demo Shiv12
...
class GoalControl(threading.Thread):
    def __init__(self, threadID):
        ...
        self.min_linearVelocity = rospy.get_param('~min_linearVelocity', 0.03) # 0.008
        ...
        self.listVel = [0.03, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.9, 1.0, 1.1]
        self.numTable = len(self.listVel)
        self.listLookAhead = [0.12, 0.22, 0.3, 0.4, 0.55, 0.65, 0.7, 0.8, 0.9, 1.05, 1.2, 1.35, 1.45, 1.50, 1.6, 1.7, 1.8, 1.9, 2.05]
    ...
    def getVeloctity(self, _safety, _statusVel):
        ...
        # else:      
            if _statusVel == 0:
                ...
                return self.funcDecelerationByAcc(self.saveTimeVel, self.velSt, self.infoPathFollow.velocity, 0.05)
            elif _statusVel == 2:
                return self.funcDecelerationByAcc(self.saveTimeVel, self.velSt, self.infoPathFollow.velocity, -0.05)
            else:
                return 0.
    ...
    def checkPathOutOfRange(self, datalistPointRequestMoveNow):
        ...
        if self.findStopGoal(goalNearNow.pathID, datalistPointRequestMoveNow): # la diem stop point
            a = 0.84
            b = 1.491
            dis = self.calculate_distance(goalNearNow.X, goalNearNow.Y, self.infoGoalStop.point.position.x, self.infoGoalStop.point.position.y)
            if dis < a*disNear + b:
                return 3
    ...
    def run(self):
        ...
        while not self.shutdown_flag.is_set(): 
            ...
            # Buoc kiem tra xu ly process 
                ...
                elif stt == 5: # di chuyen huong den diem Stop Avoid
                    self.process = 100
                    print("di chuyen huong den diem Stop Avoid", self.process)
                ...
                isChangeOrReset = self.checkResetOrTargetChange(self.datalistPointRequestMoveNow)
                if isChangeOrReset== 0:
                    stt = self.moveToPoint(self.infoGoalStop.point.position.x, self.infoGoalStop.point.position.y, self.toleranceByX_PointStop, self.toleranceByY_PointStop, self.min_linearVelocity)
                    if stt == 1: # done
                        self.process = 15
                    elif stt == -1: # error
                        print("error", self.process)
            ...
            elif self.process == 100: # di chuyen den vi tri Stop Avoid
                self.datalistPointRequestMoveNow = copy.copy(self.datalistPointRequestMove)
                isChangeOrReset = self.checkResetOrTargetChange(self.datalistPointRequestMoveNow)
                if isChangeOrReset== 0:
                    stt = self.moveToPoint(self.pointStopAvoid.x, self.pointStopAvoid.y, self.toleranceByX_PointStop, self.toleranceByY_PointStop, self.min_linearVelocity)
                    if stt == 1: # done
                        self.process = 101
                    elif stt == -1: # error
                        print("error", self.process)
            ...
            if self.datalistPointRequestMoveNow.enable == 0:
                self.pub_status(self.datalistPointRequestMoveNow.enable,0,0,0,0,0)

            elif self.datalistPointRequestMoveNow.enable == 1 or self.datalistPointRequestMoveNow.enable == 3:
                self.pub_status(self.datalistPointRequestMoveNow.enable,self.process,0,self.warnAGV,self.completed_all,self.infoPathFollow.pathID)   

            elif self.datalistPointRequestMoveNow.enable == 2:
                self.pub_status(self.datalistPointRequestMoveNow.enable,self.process,0,self.warnAGV,self.completed_backward,0)   
