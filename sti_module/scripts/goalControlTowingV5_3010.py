...
# Hoang sua code cho AGV keo SHIV12 30/5/2023
# update 7/10/2024 update van toc
# update 29/10/2024 update chay bang odom
# update 30/10/2024 update chay bang guong
...
class GoalControl(threading.Thread):
    def __init__(self, threadID):
        ...
        self.topic_pose = rospy.get_param('~topic_pose', '/robotPose_lidarLOC')
        ...
        # -- update 7/10/2024
        rospy.Subscriber("/localizationcontroller/out/localizationcontroller_result_message_0502", LocalizationControllerResultMessage0502, self.callback_lls)
        self.loc_status = 0
        self.map_match_status = 0
        self.is_lls = False

        # -- update 29/10/2024
        rospy.Subscriber("/type_nav_request", Int8, self.callback_naviType)
        # self.navi_odom = 0

        rospy.Subscriber("/odom", Odometry, self.callback_Odometry) # or odom_combined

        rospy.Subscriber("/robotPose_reflector", PoseStamped, self.callback_poseReflector) # or odom_combined
        ...
        self.maxVelZone3 = 0.3
        self.maxVelZone2 = 0.2

        # -- sai so duong dan
        self.path_error = 0.

        # -- 
        self.maxVel_mapMedium = 0.2
        self.maxVel_mapLow = 0.1

        # -- 
        self.maxVel_moveByOdom = 0.2
    ...
    def callback_poseRobot(self, data):
        ...
        if self.navi_odom == 1 or self.navi_odom == 2:
            return
    ...
    def callback_Odometry(self, data):
        ...
        if self.navi_odom == 0 or self.navi_odom == 2:
            return
        
        self.poseRbMa = data.pose.pose
        quata = ( self.poseRbMa.orientation.x,\
                self.poseRbMa.orientation.y,\
                self.poseRbMa.orientation.z,\
                self.poseRbMa.orientation.w )
        ...
        print("move by odom: ", self.poseRbMa.position.x, self.poseRbMa.position.y, self.theta_robotNow)

    def callback_poseReflector(self, data):
        # if self.navi_odom == 0 or self.navi_odom == 1:
        #     return
        ...
        print("move by reflector: ", self.poseRbMa.position.x, self.poseRbMa.position.y, degrees(self.theta_robotNow))

    def callback_naviType(self, data):
        self.navi_odom = data.data

    def callback_naviReflector(self, data):
        self.navi_nect = data.data

    def callback_lls(self, data):
        self.loc_status = data.loc_status
        self.map_match_status = data.map_match_status
        self.is_lls = True
    ...
    def pub_status(self, target_x, target_y, target_z, misson, process_now, error, safety, complete_misson , id, _path_error):
        status = Status_goal_control()
        status.target_x = target_x
        status.target_y = target_y
        status.target_z = target_z
        status.misson = misson
        status.status_now = process_now
        status.ID_follow = int(id)
        status.error = error        # 0 (binh thuong) | 1 (AGV ra khỏi đường dẫn) | 2 (lỗi hệ thống) 
        status.safety = safety      # 0 (binh thuong) | 1 (có vật cản)
    ...
    def getVeloctity(self, velCmd, _statusVel):
        if _statusVel == 0:
            ...
            return self.funcDecelerationByAcc(self.saveTimeVel, self.velSt, velCmd, 0.025)
        elif _statusVel == 2:
    ...
    def infoPointPathByIndex(self, _index, datalistPointRequestMoveNow):
        ...
        infoPoint.pathID = datalistPointRequestMoveNow.infoPoint[_index].pathID
        # vel = (datalistPointRequestMoveNow.infoPoint[_index].velocity/100.)*self.max_linearVelocity
        vel = datalistPointRequestMoveNow.infoPoint[_index].velocity
        infoPoint.velocity = self.constrain(vel, self.min_linearVelocity, self.max_linearVelocity)
        infoPoint.typePath = datalistPointRequestMoveNow.infoPoint[_index].typePath
        infoPoint.direction = datalistPointRequestMoveNow.infoPoint[_index].direction
        infoPoint.movableZone = datalistPointRequestMoveNow.infoPoint[_index].movableZone
        infoPoint.fieldSafety = datalistPointRequestMoveNow.infoPoint[_index].fieldSafety
        infoPoint.X = datalistPointRequestMoveNow.infoPoint[_index].pose.position.x
        infoPoint.Y = datalistPointRequestMoveNow.infoPoint[_index].pose.position.y
    ...
    def followPath(self, _ssXNormal, _ssYNormal, _ssXFinish, _ssYFinish, datalistPointRequestMoveNow):
        ...
        if (fabs(ss_x) <= toleranceX and fabs(ss_y) <= toleranceY) or dis <= tolerance_radius or isFinish:
            ...
            if self.flagFollowPointStop:
                ...
                velCmd = 0.
                if self.directFollow == self.directForward:
                    if self.respSafety.zone_ahead == 1 or self.lostSafety or (self.map_match_status < 30 and self.navi_odom == 0):
                        ...
                        if self.isDecelerationObstacles == 1:
                            ...
                            self.saveVelWhenDecObs = self.maxVelZone2
                            self.statusVel = 0

                        elif self.isDecelerationObstacles != 2:
                            # self.saveVelWhenDecObs = self.curr_velocity*(1./3.)
                            if self.curr_velocity < self.maxVelZone2:
                                self.saveVelWhenDecObs = self.curr_velocity
                            else:
                                self.saveVelWhenDecObs = self.maxVelZone2
                    ...
                    # elif self.map_match_status == 30:
                    #     if self.curr_velocity < self.maxVel_mapLow:
                    #         velCmd = self.curr_velocity

                    #     else:
                    #         velCmd = self.maxVel_mapLow

                    #     if velCmd <= self.min_linearVelocity:
                    #         velCmd = self.min_linearVelocity
                        
                    #     print('in low matching!')

                    # elif self.map_match_status == 60:
                    #     if self.curr_velocity < self.maxVel_mapMedium:
                    #         velCmd = self.curr_velocity

                    #     else:
                    #         velCmd = self.maxVel_mapMedium

                    #     if velCmd <= self.min_linearVelocity:
                    #         velCmd = self.min_linearVelocity
                        
                    #     print('in medium matching!')

                    # elif self.navi_odom == 1:
                    #     if self.curr_velocity < self.maxVel_moveByOdom:
                    #         velCmd = self.curr_velocity

                    #     else:
                    #         velCmd = self.maxVel_moveByOdom

                    #     if velCmd <= self.min_linearVelocity:
                    #         velCmd = self.min_linearVelocity
                        
                    #     print('in move by odom!')
                        ...
                        if self.map_match_status == 30 and self.navi_odom == 0:
                            if self.curr_velocity < self.maxVel_mapLow:
                                velCmd = self.curr_velocity
                        ...
                        elif self.map_match_status == 60 and self.navi_odom == 0:
                            if self.curr_velocity < self.maxVel_mapMedium:
                                velCmd = self.curr_velocity
                        ...
                        elif self.navi_odom == 2:
                            if self.curr_velocity < 0.3:
                                velCmd = self.curr_velocity
                ...
                print(self.curr_velocity, self.statusVel, self.infoPathFollow.velocity, velCmd)
    ...
    def checkResetOrTargetChange(self, datalistPointRequestMoveNow):
        ...
        if datalistPointRequestMoveNow.enable == 0 or self.target_x != targetX or self.target_y != targetY or self.enb == 0:
            self.stop()
            return 1
    ...
    def run(self):
        ...
        while not self.shutdown_flag.is_set(): 
            datalistPointRequestMoveNow = copy.copy(self.datalistPointRequestMove)

            # print("-----------------------------ffff-------", datalistPointRequestMoveNow.enable)
            ...
            if self.process == 0:
                ...
                if self.is_listPointRequestMove:
                    # datalistPointRequestMoveNow = copy.copy(self.datalistPointRequestMove)
                    if self.enb == 0:
                        print("co tin hieu reset",  datalistPointRequestMoveNow.enable, self.enb)
                        
                    if datalistPointRequestMoveNow.enable == 0 or self.enb == 0: # tin hieu reset
                    ...
                    elif datalistPointRequestMoveNow.enable == 1: # di chuyen theo line
                        self.process = 19

                    elif datalistPointRequestMoveNow.enable == 2: # di chuyen ra khoi pallet
                        self.process = 22
            ...
            ## them buoc cap nhat target and list Move
                ...
                targetY = round(datalistPointRequestMoveNow.target_y, 3)
                targetZ = round(datalistPointRequestMoveNow.target_z, 3)
                if datalistPointRequestMoveNow.enable == 1:
            ...
            # Buoc kiem tra xu ly process 
                ...
                # datalistPointRequestMoveNow = copy.copy(self.datalistPointRequestMove)
                stt = self.checkPathOutOfRange(datalistPointRequestMoveNow)
                if stt == -1:
                    self.process = 1
                    print("OUT OF PATH")
                ...
                # datalistPointRequestMoveNow = copy.copy(self.datalistPointRequestMove)
                isChangeOrReset = self.checkResetOrTargetChange(datalistPointRequestMoveNow)
                ...
                if datalistPointRequestMoveNow.enableStop == 0:
                ...
                isChangeOrReset = self.checkResetOrTargetChange(datalistPointRequestMoveNow)
                if isChangeOrReset== 0:
                    stt = self.moveIntoPath(datalistPointRequestMoveNow)
                    if stt == 1:
                ...
                if isChangeOrReset== 0:
                    ...
                    if datalistPointRequestMoveNow.enableStop == 1:
                        print("dung tai day")
                        self.statusVel = 0
                        self.pointStopAvoid.x = datalistPointRequestMoveNow.pointStopAvoid.point.position.x
                        self.pointStopAvoid.y = datalistPointRequestMoveNow.pointStopAvoid.point.position.y
                        self.stop()

                    else:
                        stt = self.followPath(self.toleranceByX_PointStop, self.toleranceByY_PointStop, self.toleranceByX_PointFinish, self.toleranceByY_PointFinish, datalistPointRequestMoveNow)
                ...
                if self.completed_backward == 0:
                    self.completed_reset = False
                    if self.statusAGV == 0:
                        if datalistPointRequestMoveNow.target_x < 500 and datalistPointRequestMoveNow.target_y < 500:
                            self.target_x = datalistPointRequestMoveNow.target_x
                            self.target_y = datalistPointRequestMoveNow.target_y
                            self.process = 23
                            self.statusAGV = 1
                ...
                if datalistPointRequestMoveNow.enable == 0:
                    self.process = 1
            ...
            if datalistPointRequestMoveNow.enable == 0 or self.enb == 0:
                self.fieldRequest.data = 0
                self.pub_status(self.target_x, self.target_y, self.target_z, datalistPointRequestMoveNow.enable,0,0,0,0,0,0.)

            elif datalistPointRequestMoveNow.enable == 1 or datalistPointRequestMoveNow.enable == 3:
                self.fieldRequest.data = self.infoGoalNearest.fieldSafety
                self.pub_status(self.target_x, self.target_y, self.target_z, datalistPointRequestMoveNow.enable,self.process,self.errorAGV,self.sSafety_AGV,self.completed_all,self.infoPathFollow.pathID, self.path_error)   

            elif datalistPointRequestMoveNow.enable == 2:
                self.fieldRequest.data = self.infoGoalNearest.fieldSafety
                self.pub_status(self.target_x, self.target_y, self.target_z, datalistPointRequestMoveNow.enable,self.process,self.errorAGV,self.sSafety_AGV,self.completed_backward,0, 0.)   
